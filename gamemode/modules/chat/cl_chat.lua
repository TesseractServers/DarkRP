...
local function AddToChat(bits)
    ...
    if text and text ~= "" then
        ...
        if shouldShow ~= true then
            chat.AddNonParsedText(Color(241, 172, 101, 255), prefixText, Color(255, 255, 151, 255), ": " .. text)
        end
    else
        shouldShow = hook.Call("ChatText", GAMEMODE, "0", prefixText, prefixText, "darkrp")

        if shouldShow ~= true then
            chat.AddNonParsedText(Color(241, 172, 101, 255), prefixText)
        end
    end
    chat.PlaySound()
end
